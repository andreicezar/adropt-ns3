                                  ...
                                  Ptr<NetworkStatus> networkStatus)
{
    ...
    // Get the latest received packet info - check if list exists and is not empty
    ...
    NS_LOG_DEBUG("ADRopt: Device " << deviceAddr << " now has " 
                 << deviceStats.packetHistory.size() << " packets in history");
}
                                   ...
                                   Ptr<NetworkStatus> networkStatus)
{
    ...
    if (parametersChanged)
    {
        NS_LOG_INFO("ADRopt: New parameters for device " << deviceAddr 
                    << " - DR: " << (uint32_t)newDataRate 
                    << ", TxPower: " << newTxPower 
                    << ", NbTrans: " << (uint32_t)newNbTrans);
                                   ...
                                   Ptr<EndDeviceStatus> status)
{
    ...
    NS_LOG_DEBUG("ADRopt: Current config - DR: " << (uint32_t)currentDR 
                 << ", TxPower: " << currentTxPower 
                 << ", NbTrans: " << (uint32_t)currentNbTrans);
    ...
    for (uint8_t dr = 0; dr <= 5; ++dr)
    {
        ...
        for (double power = startPower; power <= endPower; power += 2.0)
        {
            ...
            for (uint8_t nbt = 1; nbt <= 3; ++nbt)
            {
                ...
                NS_LOG_DEBUG("ADRopt: Testing DR" << (uint32_t)dr 
                            << ", Power:" << power 
                            << ", NbTrans:" << (uint32_t)nbt 
                            << " -> PER:" << predictedPER 
                            << ", ToA:" << toa);
